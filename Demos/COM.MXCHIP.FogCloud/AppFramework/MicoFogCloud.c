...
void MicoFogCloudDevCloudInfoResetThread(void *arg)
{
  ...
  fogcloud_log("MicoFogCloudDevCloudInfoResetThread: entry");
...
void MicoFogCloudMainThread(void *arg)
{
  ...
  fogcloud_log("MicoFogCloudMainThread: _wifi_station_on_sem true");
  ...
  if((inContext->flashContentInRam.appConfig.fogcloudConfig.needCloudReset) && 
     (inContext->flashContentInRam.appConfig.fogcloudConfig.isActivated)){
       ...
       fogcloud_log("MicoFogCloudMainThread: create MicoFogCloudDevCloudInfoResetThread thread");
  ...
  fogcloud_log("MicoFogCloudMainThread: DISABLE_FOGCLOUD_AUTO_ACTIVATE is not defined");
  while(false == inContext->flashContentInRam.appConfig.fogcloudConfig.isActivated){
    fogcloud_log("MicoFogCloudMainThread: isActivated is false");
    ...
    strncpy(devDefaultActivateData.loginId,
            inContext->flashContentInRam.appConfig.fogcloudConfig.loginId,
            MAX_SIZE_LOGIN_ID);
    fogcloud_log("MicoFogCloudMainThread: loginId: %s", devDefaultActivateData.loginId);
    
    strncpy(devDefaultActivateData.devPasswd,
            inContext->flashContentInRam.appConfig.fogcloudConfig.devPasswd,
            MAX_SIZE_DEV_PASSWD);
    fogcloud_log("MicoFogCloudMainThread: devPasswd: %s", devDefaultActivateData.devPasswd);
    
    strncpy(devDefaultActivateData.user_token,
            inContext->micoStatus.mac,
            MAX_SIZE_USER_TOKEN);
    fogcloud_log("MicoFogCloudMainThread: user_token: %s", devDefaultActivateData.user_token);

    fogcloud_log("MicoFogCloudMainThread: start fogCloudDevActivate()");
    err = fogCloudDevActivate(inContext, devDefaultActivateData);
    fogcloud_log("MicoFogCloudMainThread: fogCloudDevActivate() finished");
    if(kNoErr == err){
...
OSStatus MicoStartFogCloudService(mico_Context_t* const inContext)
{
  ...
  fogcloud_log("MicoStartFogCloudService: entry");
  
  //init cloud service interface
  fogcloud_log("MicoStartFogCloudService: fogCloudInit() start");
  err = fogCloudInit(inContext);
  fogcloud_log("MicoStartFogCloudService: fogCloudInit() finished");
  require_noerr_action(err, exit, 
  ...
  fogcloud_log("MicoStartFogCloudService: create MicoFogCloudMainThread thread");
